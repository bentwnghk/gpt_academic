...
def predict_no_ui_long_connection(inputs:str, llm_kwargs:dict, history:list, sys_prompt:str, observe_window:list=[], console_slience:bool=False):
    ...
    if '&' not in model:
        ...
        def mutex_manager(window_mutex, observe_window):
            while True:
                ...
                for i in range(n_model):
                    ...
                    chat_string.append( f"【{str(models[i])} 說】: <font color=\"{color}\"> {window_mutex[i][0]} </font>" )
                res = '<br/><br/>\n\n---\n\n'.join(chat_string)
                # # # # # # # # # # #
        ...
        for i, future in enumerate(futures):  # wait and get
            color = colors[i%len(colors)]
            return_string_collect.append( f"【{str(models[i])} 說】: <font color=\"{color}\"> {future.result()} </font>" )
