...
class NewBingHandle(Process):
    ...
    def run(self):
        ...
        try:
            ...
            tb_str = "\n```\n" + trimmed_format_exc() + "\n```\n"
            self.child.send(
                f"[Local Message] Newbing 請求失敗，報錯資訊如下. 如果是與網路相關的問題，建議更換代理協定（推薦http）或代理節點 {tb_str}."
            )
            self.child.send("[Fail]")
            self.child.send("[Finish]")
...
def predict_no_ui_long_connection(
    ...
    response = ""
    if len(observe_window) >= 1:
        observe_window[0] = "[Local Message] 等待NewBing響應中 ..."
...
def predict(
    ...
    yield from update_ui(
        chatbot=chatbot, history=history, msg="NewBing響應緩慢，尚未完成全部響應，請耐心完成後再提交新問題。"
    )
    for response in newbingfree_handle.stream_chat(
        query=inputs,
        history=history_feedin,
    ...
    if response == "[Local Message] 等待NewBing響應中 ...":
