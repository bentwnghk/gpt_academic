...
def update_ui(chatbot:ChatBotWithCookies, history, msg="正常", **kwargs):  # 刷新界面
    ...
    ), "在傳遞chatbot的過程中不要將其丟棄。必要時，可用clear將其清空，然後用for+append循環重新賦值。"
...
def CatchException(f):
    ...
    def decorated(main_input:str, llm_kwargs:dict, plugin_kwargs:dict,
        ...
        try:
            ...
            chatbot_with_cookie[-1] = (chatbot_with_cookie[-1][0], f"[Local Message] 插件調用出錯: \n\n{tb_str} \n")
...
def on_file_uploaded(
    ...
    chatbot.append(
        [
            "我上傳了文件，請查收",
            f"[Local Message] 收到以下文件 (上傳到路徑：{target_path_base}）: " +
            f"\n\n{moved_files_str}" +
            f"\n\n調用路徑參數已自動修正到: \n\n{txt}" +
            f"\n\n現在您點擊任意函數插件時，以上文件將被作為輸入參數" +
            upload_msg,
        ]
    )

    txt, txt2 = target_path_base, ""
    if "浮動輸入區" in checkboxes:
        txt, txt2 = txt2, txt
...
def on_report_generated(cookies:dict, files:List[str], chatbot:ChatBotWithCookies):
    ...
    chatbot.append(["報告如何遠程獲取？", f"報告已經添加到右側“文件下載區”（可能處於折疊狀態），請查收。{file_links}"])
