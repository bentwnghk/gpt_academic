...
def 解析PDF(file_manifest, project_folder, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt):
    ...
    for file_name in file_manifest:
        ...
        i_say_show_user = f'首先你在中文語境下通讀整篇論文。'; gpt_say = "[Local Message] 收到。"           # 用户提示
...
do not have too much repetitive information, numerical values using the original numbers.
        ...
        gpt_say = yield from request_gpt_model_in_new_thread_with_ui_alive(
            inputs=i_say, inputs_show_user='開始最終總結', 
...
def 批量总结PDF文档(txt, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt, web_port):
    ...
    chatbot.append([
        "函數插件功能？",
        "批量總結PDF文檔。函數插件貢獻者: ValeriaWong，Eralien"])
    ...
    if len(file_manifest) == 0:
        report_execption(chatbot, history, a = f"解析項目: {txt}", b = f"找不到任何.tex或.pdf文件: {txt}")
        yield from update_ui(chatbot=chatbot, history=history) # 刷新界面
        return
