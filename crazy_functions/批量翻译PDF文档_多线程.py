...
def 批量翻译PDF文档(txt, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt, web_port):
    ...
    try:
        ...
        report_execption(chatbot, history,
                         a=f"解析項目: {txt}",
                         b=f"導入軟件依賴失敗。使用該模塊需要額外依賴，安裝方法```pip install --upgrade pymupdf tiktoken scipdf_parser```。")
        yield from update_ui(chatbot=chatbot, history=history) # 刷新界面
        return
    ...
    if grobid_url is not None:
        ...
        yield from update_ui_lastest_msg("GROBID服務不可用，請檢查config中的GROBID_URL。作為替代，現在將執行效果稍差的舊版代碼。", chatbot, history, delay=3)
...
def 解析PDF_基于GROBID(file_manifest, project_folder, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt, grobid_url):
    ...
    for index, fp in enumerate(file_manifest):
        ...
        if article_dict is None: raise RuntimeError("解析PDF失敗，請檢查PDF是否損壞。")
        prompt = "以下是一篇學術論文的基本信息:\n"
        # title
        title = article_dict.get('title', '無法獲取 title'); prompt += f'title:{title}\n\n'
        # authors
        authors = article_dict.get('authors', '無法獲取 authors'); prompt += f'authors:{authors}\n\n'
        # abstract
        abstract = article_dict.get('abstract', '無法獲取 abstract'); prompt += f'abstract:{abstract}\n\n'
        # command
        prompt += f"請將題目和摘要翻譯為{DST_LANG}。"
        meta = [f'# Title:\n\n', title, f'# Abstract:\n\n', abstract ]
        ...
        gpt_response_collection = yield from request_gpt_model_multi_threads_with_very_awesome_ui_and_high_efficiency(
            ...
            sys_prompt_array=[
                "請你作為一個學術翻譯，負責把學術論文準確翻譯成中文。注意文章中的每一句話都要翻譯。" for _ in inputs_array],
        )
        ...
        final = ["", "", "一、論文概況",  "", "Abstract", paper_meta_info,  "二、論文翻譯",  ""]
...
def 解析PDF(file_manifest, project_folder, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt):
    ...
    for pdf_path in generated_conclusion_files:
        # 重命名文件
        rename_file = f'翻譯-{os.path.basename(pdf_path)}'
        promote_file_to_downloadzone(pdf_path, rename_file=rename_file, chatbot=chatbot)
    for html_path in generated_html_files:
        # 重命名文件
        rename_file = f'翻譯-{os.path.basename(html_path)}'
