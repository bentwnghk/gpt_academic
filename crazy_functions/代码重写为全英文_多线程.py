...
def 全项目切换英文(txt, llm_kwargs, plugin_kwargs, chatbot, history, sys_prompt, web_port):
    ...
    for index, fp in enumerate(file_manifest):
        ...
        with open(fp, 'r', encoding='utf-8', errors='replace') as f:
            file_content = f.read()
        i_say_show_user =f'[{index}/{len(file_manifest)}] 接下來請將以下代碼中包含的所有中文轉化為英文，只輸出轉化後的英文代碼，請用代碼塊輸出代碼: {os.path.abspath(fp)}'
        i_say_show_user_buffer.append(i_say_show_user)
        chatbot.append((i_say_show_user, "[Local Message] 等待多線程操作，中間過程不予顯示"))
        yield from update_ui(chatbot=chatbot, history=history) # 刷新界面
    ...
    def thread_worker(fp,index):
        if index > 10: 
            time.sleep(60)
            print('Openai 限制免費用戶每分鐘20次請求，降低請求頻率中。')
        with open(fp, 'r', encoding='utf-8', errors='replace') as f:
            file_content = f.read()
        i_say_template = lambda fp, file_content: f'接下來請將以下代碼中包含的所有中文轉化為英文，只輸出代碼，文件名是{fp}，文件代碼是 ```{file_content}```'
        try:
            gpt_say = ""
            # 分解代码文件
            ...
            print('至少一個線程任務Token溢出而失敗', e)
        except Exception as e:
            print('至少一個線程任務意外失敗', e)
    ...
    for h in handles:
        h.daemon = True
        h.start()
    chatbot.append(('開始了嗎？', f'多線程操作已經開始'))
    yield from update_ui(chatbot=chatbot, history=history) # 刷新界面
    ...
    for index, h in enumerate(handles):
        ...
        chatbot.append((i_say_show_user, f'[Local Message] 已完成{os.path.abspath(fp)}的轉化，\n\n存入{os.path.abspath(where_to_relocate)}'))
