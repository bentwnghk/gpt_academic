...
def 虚空终端(txt, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt, web_port):
    ...
    if is_certain or (state.has_provided_explaination):
        ...
        chatbot.append(("虛空終端狀態:", explain_msg+appendix_msg))
        yield from update_ui(chatbot=chatbot, history=history)
        return
...
def 虚空终端主路由(txt, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt, web_port):
    history = []
    chatbot.append(("虛空終端狀態: ", f"正在執行任務: {txt}"))
    yield from update_ui(chatbot=chatbot, history=history) # 刷新界面
    ...
    if not is_certain:
        yield from update_ui_lastest_msg(
            lastmsg=f"正在執行任務: {txt}\n\n分析用戶意圖中", chatbot=chatbot, history=history, delay=0)
        ...
        try:
            ...
            yield from update_ui_lastest_msg(
                lastmsg=f"正在執行任務: {txt}\n\n用戶意圖理解: 失敗 當前語言模型（{llm_kwargs['llm_model']}）不能理解您的意圖", chatbot=chatbot, history=history, delay=0)
            return
    else:
        pass
