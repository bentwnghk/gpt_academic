...
def get_crazy_functions():
    ...
    function_plugins.update({
        "詢問多個GPT模型（手動指定詢問哪些模型）": {
            ...
            "AsButton": False,
            "AdvancedArgs": True, # 调用时，唤起高级参数输入区（默认False）
            "ArgsReminder": "支持任意數量的LLM接口，用&符號分隔。例如: chatglm&gpt-3.5-turbo&api2d-gpt-4", # 高级参数输入区的显示提示
    ...
    function_plugins.update({
        "圖片生成（先切換模型到openai或api2d）": {
            ...
            "AsButton": False,
            "AdvancedArgs": True, # 调用时，唤起高级参数输入区（默认False）
            "ArgsReminder": "在這裡輸入分辨率，例如: 256x256（默認）", # 高级参数输入区的显示提示
    ...
    function_plugins.update({
        "批量總結音視頻（輸入路徑或上傳壓縮包）": {
            ...
            "AsButton": False,
            "AdvancedArgs": True,
            "ArgsReminder": "調用openai api 使用whisper-1模型, 目前支持的格式:mp4, m4a, wav, mpga, mpeg, mp3。此處可以輸入解析提示，例如：解析為繁體中文（默認）。",
