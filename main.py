    ...
    with gr.Blocks(title="Mr.🆖 GPT 學術優化", theme=set_theme, analytics_enabled=False, css=advanced_css) as app_block:
        ...
        with gr_L1():
            with gr_L2(scale=2, elem_id="gpt-chat"):
                chatbot = gr.Chatbot(label=f"當前模型：{LLM_MODEL}", elem_id="gpt-chatbot")
                ...
                with gr.Accordion("函數插件區", open=True, elem_id="plugin-panel") as area_crazy_fn:
                    ...
                    with gr.Row():
                        with gr.Accordion("更多函數插件", open=True):
                            ...
                            with gr.Row():
                                dropdown = gr.Dropdown(dropdown_fn_list, value=r"打開插件列表", label="", show_label=False).style(container=False)
                            with gr.Row():
        ...
        with gr.Floating(init_x="0%", init_y="0%", visible=True, width=None, drag="forbidden", elem_id="tooltip"):
            with gr.Row():
                ...
                with gr.Tab("更換模型", elem_id="interact-panel"):
                    md_dropdown = gr.Dropdown(AVAIL_LLM_MODELS, value=LLM_MODEL, elem_id="elem_model_sel", label="更換AI/LLM模型").style(container=False)
                ...
                with gr.Tab("介面外觀", elem_id="interact-panel"):
                    ...
                    checkboxes = gr.CheckboxGroup(["基礎功能區", "函數插件區", "浮動輸入區", "輸入清除鍵", "插件參數區"], value=["基礎功能區", "插件參數區"], label="顯示/隱藏功能區", elem_id='cbs').style(container=False)
                    opt = ["自訂選單"]
                    value=[]
                    if ADD_WAIFU: opt += ["添加Live2D形象"]; value += ["添加Live2D形象"]
                    checkboxes_2 = gr.CheckboxGroup(opt, value=value, label="顯示/隱藏自訂選單", elem_id='cbsc').style(container=False)
        ...
        with gr.Floating(init_x="20%", init_y="50%", visible=False, width="40%", drag="top") as area_input_secondary:
            with gr.Accordion("浮動輸入區", open=True, elem_id="input-panel2"):
                with gr.Row() as row:
        ...
        with gr.Floating(init_x="20%", init_y="50%", visible=False, width="40%", drag="top") as area_customize:
            with gr.Accordion("自訂選單", open=True, elem_id="edit-panel"):
                with gr.Row() as row:
                    with gr.Column(scale=10):
        ...
        def fn_area_visibility(a):
            ret = {}
            ret.update({area_input_primary: gr.update(visible=("浮動輸入區" not in a))})
            ret.update({area_input_secondary: gr.update(visible=("浮動輸入區" in a))})
        ...
        def fn_area_visibility_2(a):
            ret = {}
            ret.update({area_customize: gr.update(visible=("自訂選單" in a))})
            return ret
        ...
        def on_dropdown_changed(k):
            ...
            info = plugins[k].get("Info", k)
            ret = {switchy_bt: gr.update(value=k, variant=variant, info_str=f'函數插件區: {info}')}
            if plugins[k].get("AdvancedArgs", False): # 是否唤起高级插件参数区
                ret.update({plugin_advanced_arg: gr.update(visible=True,  label=f"插件[{k}]的高級參數說明：" + plugins[k].get("ArgsReminder", [f"沒有提供高級參數功能說明"]))})
            else:
